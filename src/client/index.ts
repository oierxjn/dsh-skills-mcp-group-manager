/**
 * dsh-skills-mcp-group-manager — client half (browser), TypeScript source.
 *
 * Built by tsdown into the classic client-modules script `lib/client.js`
 * (`window.__ModuleLoader__.load({ id, factory })`; see tsdown.config.ts for
 * the banner/footer/intro). The factory is CJS-style and may require only
 * platform seed words (`react`); everything else is self-contained. The
 * exports are the Cordis plugin face: `{ apply, inject }` plus `__logic` for
 * unit tests.
 *
...
import type { ManagerState, ProbeOutcome, ServerStatus, SkillGroup } from '../types.ts'

// The loose createElement signature gives inline event handlers contextual
// `any` (onChange/onKeyDown arrows stay untyped, preserving the built
// bundle's `(event: any) =>` shapes) while keeping every other position checked.
...
function dictFor(): LocaleDictionary {
  const lang = (typeof document !== 'undefined' && document.documentElement && document.documentElement.lang) || 'en'
  return lang.toLowerCase().startsWith('zh') ? zh : en
}
function makeT(t: unknown): Translate {
  if (typeof t === 'function') return t as Translate
  const dict = dictFor()
  return (key, params) => {
...
// ── RPC client ─────────────────────────────────────────────────────────────
/** Structured host RPC error: { code, message, fields? } (fields = inline form errors). */
...
async function rpc(method: string, args?: Record<string, unknown>): Promise<any> {
  ...
  if (!data.ok) {
    ...
    const message = (typeof raw === 'object' && raw !== null
      ...
      : raw) ?? `RPC ${method} failed`
    const error = new Error(message as string | undefined) as RpcError
    if (typeof raw === 'object' && raw !== null) {
      if (typeof (raw as { code?: unknown }).code === 'string') {
        error.code = (raw as { code: string }).code
      }
      const fields = (raw as { fields?: unknown }).fields
      if (fields !== undefined) error.fields = fields as Record<string, string>
    }
    throw error
  }
  return data.value
...
function GearIcon({ size }: { size: number }): unknown {
...
function ChevronIcon({ size, direction }: { size: number; direction?: string }): unknown {
  ...
  const d = paths[direction ?? 'down'] ?? paths.down
...
function PlusIcon({ size }: { size: number }): unknown {
...
function XIcon({ size }: { size: number }): unknown {
  return h('svg', { width: size, height: size, viewBox: '0 0 12 12', fill: 'none', stroke: 'currentColor', strokeWidth: '1.5', strokeLinecap: 'round', 'aria-hidden': true },
    h('path', { d: 'M3 3l6 6M9 3l-6 6' }),
  )
}
function PencilIcon({ size }: { size: number }): unknown {
...
/** A catalog skill as the picker consumes it (name + optional description). */
...
function filterCandidates(skills: SkillEntry[], members: string[], query: string): SkillEntry[] {
...
function addSelectedToGroup(members: string[], selected: Set<string>): string[] {
  const next = [...members]
  for (const name of selected) if (!next.includes(name)) next.push(name)
  return next
}
/** Remove exactly the selected names from the member list. */
function removeSelectedFromGroup(members: string[], selected: Set<string>): string[] {
...
interface GroupCardProps {
  ...
  onAddSkills: (id: string, names: string[]) => void
  onRemoveSkills: (id: string, names: string[]) => void
}

function GroupCard({ t, group, skills, busy, onDelete, onRename, onSetEnabled, onAddSkills, onRemoveSkills }: GroupCardProps): unknown {
  ...
  return h('div', { className: 'msm-group', 'data-expanded': expanded || undefined },
    h('div', { className: 'msm-group-row' },
      ...
      h('input', {
        ...
        disabled: busy,
        onChange: (event: any) => onSetEnabled(event.target.checked),
    ...
    expanded && h(GroupDetail, {
      ...
      skills,
      busy,
      onAddSkills: (names: any) => onAddSkills(group.id, names),
      onRemoveSkills: (names: any) => onRemoveSkills(group.id, names),
    }),
  )
}
...
interface GroupDetailProps {
  ...
  onRemoveSkills: (names: string[]) => void
}

function GroupDetail({ t, group, skills, busy, onAddSkills, onRemoveSkills }: GroupDetailProps): unknown {
  const [query, setQuery] = react.useState('')
  const [memberQuery, setMemberQuery] = react.useState('')
  const [picked, setPicked] = react.useState<Set<string>>(() => new Set<string>())
  const [removing, setRemoving] = react.useState<Set<string>>(() => new Set<string>())
    ...
    h('div', { className: 'msm-detail-head' },
      ...
      h('div', { className: 'msm-bulk-actions' },
        ...
        h('button', {
          ...
          onClick: () => { onRemoveSkills([...removing]); setRemoving(new Set<string>()) },
    ...
    h('div', { className: 'msm-detail-head' },
      ...
      h('div', { className: 'msm-bulk-actions' },
        ...
        h('button', {
          ...
          onClick: () => { onAddSkills([...picked]); setPicked(new Set<string>()) },
...
function GroupSection({ t, state, skills, busy, onCreate, onDelete, onRename, onSetEnabled, onAddSkills, onRemoveSkills }: GroupSectionProps): unknown {
  ...
  return h('div', { className: 'msm-section' },
    h('div', { className: 'msm-create-row' },
      h('input', {
        ...
        onKeyDown: (event: any) => { if (event.key === 'Enter') submitCreate() },
      }),
      h('button', {
        type: 'button',
    ...
    state.groups.length === 0
      ...
      : h('div', { className: 'msm-group-list' },
          state.groups.map((group) => h(GroupCard, {
            ...
            onRename: (name: any) => onRename(group.id, name),
            onSetEnabled: (enabled: any) => onSetEnabled(group.id, enabled),
            ...
            onAddSkills: (id: any, names: any) => onAddSkills(id, names),
            onRemoveSkills: (id: any, names: any) => onRemoveSkills(id, names),
          })),
        ),
  )
}
...
function statusOf(server: ServerStatus): { tone: string; key: string; count?: number } {
  ...
  switch (server.fiberPhase) {
    case 'active':
      ...
      // so zero tools never renders green.
...
function targetOf(server: ServerStatus): string {
...
/** The add/edit form's editable text fields (config text is flattened). */
interface McpForm {
  id: string
  serverName: string
...
/** The normalized config sent over the RPC route (id travels separately). */
interface McpConfig {
  serverName: string
  transport: 'stdio' | 'streamable-http'
...
/** The add/update payload: config fields plus the leading loader-entry id. */
interface McpPayload extends McpConfig {
  id: string
}

/** A save failure handed back to the form (message + inline field errors). */
interface SaveError {
  message?: unknown
  fields?: Record<string, string> | undefined
}
...
function formOfServer(server: ServerStatus | undefined): McpForm {
...
function configOfForm(form: McpForm): McpConfig {
  const config: McpConfig = { serverName: form.serverName.trim(), transport: form.transport }
...
// (opens in place of the card, id locked). Field-level errors come from local
// validation and from the host's structured RPC error (`fields`).
interface McpServerFormProps {
  t: Translate
  initial?: ServerStatus | undefined
  servers: ServerStatus[]
  busy: boolean
  onCancel: () => void
  onSave: (editing: boolean, payload: McpPayload) => Promise<SaveError | undefined>
}

function McpServerForm({ t, initial, servers, busy, onCancel, onSave }: McpServerFormProps): unknown {
  const editing = initial !== undefined
  const [form, setForm] = react.useState<McpForm>(() => formOfServer(initial))
  const [fieldErrors, setFieldErrors] = react.useState<Record<string, string>>({})
  const [submitError, setSubmitError] = react.useState<string | null>(null)
  const [saving, setSaving] = react.useState(false)
  const set = (key: string, value: unknown) => setForm((prev) => ({ ...prev, [key]: value }) as McpForm)
  const err = (key: string): string | undefined => fieldErrors[key]
  const inputClass = (key: string): string => `msm-input${err(key) !== undefined ? ' msm-input-invalid' : ''}`
  ...
  const field = (key: string, labelKey: string, input: unknown): unknown =>
  ...
  return h('div', { className: 'msm-form' },
    ...
    field('id', 'mcp.formId', h('input', {
      ...
      onChange: (event: any) => set('id', event.target.value),
    })),
    field('serverName', 'mcp.formName', h('input', {
      ...
      spellCheck: false,
      disabled,
      onChange: (event: any) => set('serverName', event.target.value),
    })),
    h('div', { className: 'msm-field' },
      ...
      h('select', {
        ...
        disabled,
        onChange: (event: any) => set('transport', event.target.value),
      },
    ...
    form.transport === 'streamable-http'
      ? field('url', 'mcp.formUrl', h('input', {
          ...
          spellCheck: false,
          disabled,
          onChange: (event: any) => set('url', event.target.value),
        }))
      : h(react.Fragment, null,
          field('command', 'mcp.formCommand', h('input', {
            ...
            spellCheck: false,
            disabled,
            onChange: (event: any) => set('command', event.target.value),
          })),
          field('args', 'mcp.formArgs', h('textarea', {
            ...
            spellCheck: false,
            disabled,
            onChange: (event: any) => set('argsText', event.target.value),
          })),
          field('env', 'mcp.formEnv', h('textarea', {
            ...
            spellCheck: false,
            disabled,
            onChange: (event: any) => set('envText', event.target.value),
          })),
          field('cwd', 'mcp.formCwd', h('input', {
            ...
            spellCheck: false,
            disabled,
            onChange: (event: any) => set('cwd', event.target.value),
          })),
        ),
    field('headers', 'mcp.formHeaders', h('textarea', {
      ...
      spellCheck: false,
      disabled,
      onChange: (event: any) => set('headersText', event.target.value),
    })),
    field('toolCallTimeoutMs', 'mcp.formTimeout', h('input', {
      ...
      placeholder: '60000',
      disabled,
      onChange: (event: any) => set('toolCallTimeoutMs', event.target.value.replace(/[^0-9]/g, '')),
    })),
    h('label', { className: 'msm-check' },
      h('input', {
        ...
        disabled,
        onChange: (event: any) => set('failOnStartupError', event.target.checked),
      }),
...
interface McpSectionProps {
  ...
  onRemove: (server: ServerStatus) => void
  onSave: (editing: boolean, payload: McpPayload) => Promise<SaveError | undefined>
  onProbe: (id: string) => Promise<ProbeOutcome | undefined>
}

function McpSection({ t, servers, busy, onToggle, onRemove, onSave, onProbe }: McpSectionProps): unknown {
  const [adding, setAdding] = react.useState(false)
  const [editingId, setEditingId] = react.useState<string | null>(null)
  const [probes, setProbes] = react.useState<Record<string, ProbeOutcome>>({})
  ...
  const card = (server: ServerStatus): unknown => {
...
// optimistic-mutate machinery; each settings section mounts its own instance
// (root scope, one settings page visible at a time).
function useManagerData() {
  const [status, setStatus] = react.useState<'loading' | 'ready' | 'error'>('loading')
  const [loadError, setLoadError] = react.useState<string | null>(null)
  const [state, setState] = react.useState<ManagerState>({ groups: [], sessions: {} })
  ...
  const load = react.useCallback(async (silent: boolean): Promise<void> => {
  ...
  const mutate = async (method: string, args: Record<string, unknown>, optimistic?: () => void): Promise<void> => {
    setOpError(null)
    setBusy(true)
...
function settingsGate(t: Translate, data: ReturnType<typeof useManagerData>, content: unknown): unknown {
...
function SkillGroupsSection(props: { t?: unknown }): unknown {
  const t = makeT(props.t)
  const data = useManagerData()
  const { state, skills, busy, mutate } = data

  const createGroup = (name: string): Promise<void> => mutate('manager.groups.create', { name })
  ...
  const renameGroup = (id: string, name: string): Promise<void> => mutate('manager.groups.rename', { id, name }, () => {
    data.setState((prev) => ({ ...prev, groups: prev.groups.map((group) => (group.id === id ? { ...group, name } : group)) }))
  })
  const setGroupEnabled = (id: string, enabled: boolean): Promise<void> => mutate('manager.groups.setEnabled', { id, enabled }, () => {
  ...
  const addSkills = (id: string, names: string[]): Promise<void> => mutate('manager.groups.addSkill', { id, skills: names })
  const removeSkills = (id: string, names: string[]): Promise<void> => mutate('manager.groups.removeSkill', { id, skills: names })
...
function McpSectionPage(props: { t?: unknown }): unknown {
  const t = makeT(props.t)
  const data = useManagerData()
  const { servers, busy } = data

  // MCP mutations edit cordis.patch.yml and the harness hot-reloads the tree
  // asynchronously, so after the immediate re-read the section polls twice
  // more (800/2400ms) while the reload settles — the status badge and tool
  // count only flip once the new fiber is up.
  const refreshMcpSettled = (): void => {
    window.setTimeout(() => { void data.load(true) }, 800)
    window.setTimeout(() => { void data.load(true) }, 2400)
  }
  const mutateMcp = async (method: string, args: Record<string, unknown>, optimistic?: () => void): Promise<void> => {
    await data.mutate(method, args, optimistic)
    refreshMcpSettled()
  }
  const toggleMcp = (id: string, enabled: boolean): Promise<void> => mutateMcp('manager.mcp.toggle', { id, enabled }, () => {
    data.setServers((prev) => prev.map((server) => (server.id === id ? { ...server, enabled } : server)))
  })
  const removeMcp = (server: ServerStatus): void => {
  ...
  const saveMcp = async (editing: boolean, payload: McpPayload): Promise<SaveError | undefined> => {
    data.setOpError(null)
    data.setBusy(true)
    try {
      await rpc(editing ? 'manager.mcp.update' : 'manager.mcp.add', payload as unknown as Record<string, unknown>)
      await data.load(true)
      refreshMcpSettled()
      return undefined
    } catch (error) {
      return error as SaveError
    } finally {
      data.setBusy(false)
    }
  }
  /** Probe is RPC-only; the result object (or undefined on RPC error). */
  const probeMcp = async (id: string): Promise<ProbeOutcome | undefined> => {
    data.setOpError(null)
    try {
...
// The slot is session scope: props carry `sessionId` and the entry remounts
// on session switch, so no per-session state cleanup is needed. The popover
// edits ONLY this session's group selection (override ?? the global toggles)
// and can create a new group in place; global management (groups + MCP) lives
// in the two `settings.section` pages.
function SessionGroupsButton(props: { t?: unknown; sessionId?: unknown }): unknown {
  const t = makeT(props.t)
  const sessionId = props.sessionId
  const [open, setOpen] = react.useState(false)
  const [groups, setGroups] = react.useState<SkillGroup[]>([])
  const [override, setOverride] = react.useState<Set<string> | null>(null) // null = follow global; Set of ids otherwise
  ...
  react.useEffect(() => {
    ...
    const onPointerDown = (event: MouseEvent) => {
      if (rootRef.current !== null && !rootRef.current.contains(event.target as Node | null)) setOpen(false)
    }
  ...
  const save = async (next: Set<string> | null): Promise<void> => { // next: null (follow global) | Set of group ids
    setBusy(true)
    setError(null)
    try {
  ...
  const toggleGroup = (id: string): void => {
    // Leaving follow-global starts from the current effective selection, so
    // flipping one group never disturbs the others.
    const next = override === null ? new Set(effective) : new Set(override)
    if (next.has(id)) next.delete(id); else next.add(id)
    void save(next)
  }

  const isChecked = (id: string): boolean => (override === null ? effective.includes(id) : override.has(id))
  ...
  const createGroup = async (): Promise<void> => {
    ...
    try {
      ...
      // Re-align every popover slice with the host: the new group changes the
      // list AND, in follow-global mode, the effective selection it belongs
      // to — a stale effective list would render the new group unchecked and
      // drop it from the override base when follow-global is unchecked in
      // this same popover session.
      const [stateValue, sessionValue] = await Promise.all([
        rpc('manager.state.get', {}),
        rpc('manager.session.get', { sessionId }),
      ])
  ...
  // nothing per-session to edit — render nothing (global management lives in
    ...
    open && h('div', { className: 'msm-session-popover', role: 'dialog', 'aria-label': t('session.aria') },
        ...
        : h(react.Fragment, null,
            h('label', { className: 'msm-check' },
              h('input', {
                ...
                disabled: busy,
                onChange: (event: any) => { void save(event.target.checked ? null : new Set(effective)) },
              }),
      ...
      h('div', { className: 'msm-create-row' },
        h('input', {
          ...
          disabled: busy,
          onChange: (event: any) => setNewName(event.target.value),
          onKeyDown: (event: any) => { if (event.key === 'Enter') void createGroup() },
        }),
        h('button', {
          type: 'button',
          className: 'msm-button msm-button-mini msm-button-primary',
...
const PANEL_CSS = [
  ...
  // Same specificity as .msm-button:hover (0,2,0) but declared later, so this
  // wins: without an explicit background here, .msm-button:hover's light fill
  // would replace the primary blue on hover and the white label would land on
  // it, making the button visually disappear.
...
// -- "/" skill menu freshness -------------------------------------------------
// The host ui-skill slash source caches skills.list per sessionId and only
// invalidates on agent-preset switches / connection resets, so group edits
// never reach an open session's "/" menu. registerSource throws on a
// duplicate (trigger, name) pair, but live.sources holds the registered
// source by reference — rewrite its fetch paths to pull fresh through the
// connection API on every menu open. An in-flight map dedupes concurrent
// opens; a settled map feeds the lexicon. Every step is defensive: any shape
// mismatch silently keeps the host's cached behavior.
...
function patchSkillMenuFreshness(ctx: ClientPluginContext): void {
  let triggers: any
  let connection: any
  let sessionList: any
  try {
    triggers = ctx.get('inputTriggers')
    connection = ctx.get('connection')
    sessionList = ctx.get('sessions')
  } catch {
    return // service not injectable: keep host behavior
  }
  const live = triggers && triggers.live
  if (!live || !Array.isArray(live.sources)) return
  const source = live.sources.find((entry: any) => entry.trigger === '/' && entry.name === 'skill')
  if (!source || typeof source.candidates !== 'function' || source.__msmFresh) return
  ...
  const isSubagent = (sessionId: string) => (
    sessionList && typeof sessionList.subagentAddress === 'function'
      ? sessionList.subagentAddress(sessionId)
      : undefined
  )
  const inflight = new Map<string, Promise<SlashSkill[]>>()
  const settled = new Map<string, SlashSkill[]>()
  ...
  const notifyLexicon = (sessionId: string): void => {
    for (const listener of [...(lexiconListeners.get(sessionId) ?? [])]) {
      try { listener() } catch (error) {
        console.error('[mcp-skill-manager] lexicon listener failed:', error)
      }
    }
  }
  const fetchFresh = (sessionId: string): Promise<SlashSkill[]> => {
    if (isSubagent(sessionId) !== undefined) return Promise.resolve([])
    ...
    const promise = (async () => {
      try {
        ...
        if (!result || !result.ok) {
          throw new Error(`skill.list failed: ${result && result.error ? result.error.code : 'unknown'}`)
        }
        const list = (result.value && result.value.skills) || []
        settled.set(sessionId, list)
  ...
  source.candidates = async (session: any, options: any) => {
    const query = options && options.query ? options.query : ''
    const signal = options && options.signal
    // Pull fresh at the menu-open boundary (empty query = the "/" was just
    // typed) or on a cache miss; keystroke refinements filter the
    // already-fetched list locally instead of re-requesting per char.
    if (query === '' || !settled.has(session.sessionId)) {
      await fetchFresh(session.sessionId).catch(() => {})
    }
    if (signal && signal.aborted) return []
    const list = settled.get(session.sessionId) ?? []
    return list
  ...
  source.warm = (session: any) => { fetchFresh(session.sessionId).catch(() => {}) }
  source.lexicon = (session: any) => (settled.get(session.sessionId) ?? []).map((skill) => skill.name)
  source.subscribeLexicon = (session: any, listener: () => void) => {
    const key = session.sessionId
    const set = lexiconListeners.get(key) ?? new Set<() => void>()
    lexiconListeners.set(key, set)
    set.add(listener)
    return () => {
      set.delete(listener)
      if (set.size === 0) lexiconListeners.delete(key)
    }
  }
  source.__msmFresh = true
}
...
export function apply(ctx: ClientPluginContext): void {
  ...
  ctx.slots.inject('settings.section', () => ctx.slots.register({
    name: 'settings.section',
    // Unique section id: "mcp" is taken by other MCP-manager plugins (e.g.
    // @js2hou/dsh-mcp-manager) and slot entries conflict on id + priority.
  ...
  // The slash menu must reflect group edits without a page reload.
  patchSkillMenuFreshness(ctx)
}

export const inject = ['slots', 'locale', 'inputTriggers', 'connection', 'sessions']

// Pure selection logic, exported for unit tests (the browser runtime ignores
// this extra export).
export const __logic = {
  filterCandidates,
  filterMembers,
  toggleInSet,
  selectAllFiltered,
