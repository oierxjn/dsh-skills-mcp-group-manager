...
import { writeFileAtomicSync } from './store.ts'
import type { McpServerConfig, PatchEntry, PatchRow } from './types.ts'
 ...
 */
export function resolvePatchPath(options: {
  patchFile?: unknown
  profile?: unknown
...
let schemaCache: import('js-yaml').Schema | undefined
/**
 ...
 */
async function entryListSchema(): Promise<import('js-yaml').Schema> {
  ...
  const JsExprType = new yaml.Type('tag:yaml.org,2002:js', {
    ...
    predicate: isJsExpr,
    represent: (data) => (data as { __jsExpr: unknown }).__jsExpr,
  })
...
export async function writePatchList(file: string, rows: PatchRow[]): Promise<void> {
  const yaml = await loadYaml()
...
export async function editPatchList(
  file: string,
  edit: (rows: PatchRow[]) => PatchRow[] | Promise<PatchRow[]>,
): Promise<PatchRow[]> {
...
type Located =
  ...
  | { kind: 'insert'; row: PatchRow; item: PatchEntry }
  | undefined

function locate(rows: PatchRow[], id: string): Located {
 ...
 */
export function setMcpEnabled(rows: PatchRow[], id: string, enabled: boolean): PatchRow[] {
  ...
  return rows.map((row) =>
    row === found.row
      ? {
          ...row,
          insert: (row.insert as PatchEntry[]).map((entry) =>
 ...
 */
export function updateMcpConfig(rows: PatchRow[], id: string, config: McpServerConfig): PatchRow[] {
  const found = locate(rows, id)
  if (found === undefined) {
 ...
 */
export function patchEntryName(rows: PatchRow[], id: string): string | undefined {
  for (const row of rows) {
