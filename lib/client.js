 ...
 * slot (root scope) plus a per-session groups button in
 * `conversation.session.header.actions` (session scope; props carry
 ...
 * global toggles) via `manager.session.get/set`; the full panel opens from the
 * popover's manage button. The panel manages skill groups
 * (create/rename/delete/enable,
 * add/remove member skills) and MCP servers (list with live status, enable/
 * disable, add/edit/remove, on-demand connection probe) by calling the host
 * RPC route `POST /plugins/dsh-skills-mcp-group-manager/rpc` (bundle plugins
 * have no `harness.handle`/`host.call`; the host serves the same business
 * logic over one JSON POST route with body { method, args } → { ok, value } |
 * { ok: false, error: { code, message, fields? } }). MCP mutations edit the
 * profile's cordis.patch.yml and the harness hot-reloads the tree, so after
 * every mutation the panel re-reads immediately and twice delayed (800/2400ms)
 * while the HMR reload settles.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'session.empty': '还没有分组。在管理面板创建。',
      'session.openManager': '管理分组与 MCP…',
    };
    const en = {
      ...
      'session.empty': 'No groups yet. Create one in the manager panel.',
      'session.openManager': 'Manage groups & MCPs…',
    };
    ...
    function ManagerPanel(props) {
      ...
      const batchGroupSkills = async (method, id, names) => {
        ...
        const prev = { state, skills, servers };
        try {
          await rpc(method, { id, skills: names });
          await load(true);
        } catch (error) {
          setState(prev.state);
          setSkills(prev.skills);
      ...
      // tree asynchronously, so after the immediate re-read the panel polls
      ...
      const refreshMcpSettled = () => {
        ...
        window.setTimeout(() => { void load(true); }, 2400);
      };
      const mutateMcp = async (method, args, optimistic) => {
        await mutate(method, args, optimistic);
        refreshMcpSettled();
      };
            ...
            : status === 'error'
              ...
              : h(react.Fragment, null,
                  ...
                  tab === 'groups'
                    ...
                    : h(McpSection, { t, servers, busy, onToggle: toggleMcp, onRemove: removeMcp, onSave: saveMcp, onProbe: probeMcp }),
                ),
        ),
      );
    }
    ...
    // global toggles); the full manager panel opens via the manage button.
    function SessionGroupsButton(props) {
      ...
      // Without a sessionId the slot cannot scope the popover: fall back to
      // the plain panel toggle (the pre-0.5.0 behavior).
      ...
      return h('span', { className: 'msm-session-root', ref: rootRef },
        ...
        open && h('div', { className: 'msm-session-popover', role: 'dialog', 'aria-label': t('session.aria') },
          ...
          h('button', {
            type: 'button',
            className: 'msm-button msm-button-mini msm-session-manage',
            onClick: () => { setOpen(false); window.dispatchEvent(new CustomEvent(TOGGLE_EVENT)); },
          }, t('session.openManager')),
        ),
      );
    }
