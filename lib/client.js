...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // MUST stay in sync with the host's lib/state.js SERVER_NAME_PATTERN /
    // ENTRY_ID_PATTERN: this browser bundle cannot import the host module, so
    // the two domains are intentionally duplicated. If the host schema
    ...
    function SessionGroupsButton(props) {
      ...
      const createGroup = async () => {
        ...
        try {
          ...
          // Re-align every popover slice with the host: the new group changes
          // the list AND, in follow-global mode, the effective selection it
          // belongs to - a stale effective list would render the new group
          // unchecked and drop it from the override base when follow-global
          // is unchecked in this same popover session.
          const [stateValue, sessionValue] = await Promise.all([
            rpc('manager.state.get', {}),
            rpc('manager.session.get', { sessionId }),
          ]);
          setGroups(stateValue.groups ?? []);
          setOverride(sessionValue.override === null
            ? null
            : new Set(sessionValue.override.enabledGroupIds));
          setEffective(sessionValue.effectiveGroupIds ?? []);
          setNewName('');
        } catch (createError) {
    ...
    // ── plugin face ─────────────────────────────────────────────────────────
    const inject = ['slots', 'locale', 'inputTriggers', 'connection', 'sessions'];
    ...
    // opens; a settled map feeds the lexicon. Every step is defensive: any
    // shape mismatch silently keeps the host's cached behavior.
    function patchSkillMenuFreshness(ctx) {
      let triggers;
      let connection;
      let sessionList;
      try {
      ...
      const source = live.sources.find((entry) => entry.trigger === '/' && entry.name === 'skill');
      ...
      source.candidates = async (session, options) => {
        const query = options && options.query ? options.query : '';
        const signal = options && options.signal;
        // Pull fresh at the menu-open boundary (empty query = the "/" was
        // just typed) or on a cache miss; keystroke refinements filter the
      ...
      source.warm = (session) => { fetchFresh(session.sessionId).catch(() => {}); };
      source.lexicon = (session) => (settled.get(session.sessionId) ?? []).map((skill) => skill.name);
