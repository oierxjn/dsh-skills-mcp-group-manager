...
import { mcpConfigParams, parameterSchema, sessionStateOutput, valueSchema } from './tool-schemas.js';
import {
  addSkillsToGroup,
...
/** @typedef {import('./types.js').SkillsListArgs} SkillsListArgs */
/** @typedef {import('./types.js').GroupCreateArgs} GroupCreateArgs */
/** @typedef {import('./types.js').GroupIdArgs} GroupIdArgs */
/** @typedef {import('./types.js').GroupRenameArgs} GroupRenameArgs */
/** @typedef {import('./types.js').GroupSetEnabledArgs} GroupSetEnabledArgs */
/** @typedef {import('./types.js').GroupSkillsArgs} GroupSkillsArgs */
/** @typedef {import('./types.js').SessionGetArgs} SessionGetArgs */
/** @typedef {import('./types.js').SessionSetArgs} SessionSetArgs */
/** @typedef {import('./types.js').McpToggleArgs} McpToggleArgs */
/** @typedef {import('./types.js').McpAddArgs} McpAddArgs */
/** @typedef {import('./types.js').McpProbeArgs} McpProbeArgs */
...
/** @type {readonly ['webServer', 'httpServer']} */
 ...
 *
 * @param {HostPluginContext} ctx - plugin context (injects skills/tools/agents/loader).
 * @param {{ patchFile?: unknown, profile?: unknown }} [config] - row config (see above).
 */
export function apply(ctx, config = {}) {
  ...
  /** @type {Map<string, { control: SkillProviderControl, dispose: () => void }>} */
  ...
  function applyAgentFilter(agent) {
    ...
    const skills = /** @type {SkillRegistry | undefined} */ (agent.ctx.get('skills'));
    ...
    shadowControls.set(agent.id, {
      control: /** @type {SkillProviderControl} */ (controlRef),
      dispose,
    });
  }
  ...
  const api = {
     ...
     * @param {SkillsListArgs | undefined} [args]
     */
    async skillsList(args) {
      ...
      return {
        skills: all.map((skill) => ({
          ...
          invocation: {
            modelInvocable: /** @type {{ modelInvocable: boolean, userInvocable: boolean }} */ (skill.invocation).modelInvocable,
            userInvocable: /** @type {{ modelInvocable: boolean, userInvocable: boolean }} */ (skill.invocation).userInvocable,
          },
        })),
      };
    },
    /** @param {GroupCreateArgs | undefined} [args] */
    ...
    /** @param {GroupRenameArgs | undefined} [args] */
    ...
    /** @param {GroupIdArgs | undefined} [args] */
    ...
    /** @param {GroupSetEnabledArgs | undefined} [args] */
    ...
    /** @param {GroupSkillsArgs | undefined} [args] */
    ...
    /** @param {SessionGetArgs | undefined} [args] */
    ...
    /** @param {SessionSetArgs | undefined} [args] */
    ...
    /** @returns {Promise<import('./types.js').McpListResult>} */
    async mcpList() {
      const rows = await readPatchList(patchFile);
      return {
    ...
    /** @param {McpToggleArgs | undefined} [args] */
    ...
    /** @param {McpAddArgs | undefined} [args] */
    async mcpAdd(args) {
      ...
      await withWriteLock(async () => {
        ...
        if (serverNameTaken(ctx, /** @type {string} */ (config.serverName))) {
          throw new McpError('duplicate-server-name', `serverName "${config.serverName}" is already used by another MCP server`);
        }
        await editPatchList(patchFile, (current) => addMcpRow(current, id, /** @type {McpServerConfig} */ (config)));
      });
      return { id };
    },
    /** @param {McpAddArgs | undefined} [args] */
    async mcpUpdate(args) {
      ...
      await withWriteLock(async () => {
        ...
        await requireMcpTarget(id);
        // requireValidServer above guarantees serverName is a present string.
        if (serverNameTaken(ctx, /** @type {string} */ (config.serverName), id)) {
          throw new McpError('duplicate-server-name', `serverName "${config.serverName}" is already used by another MCP server`);
        }
        await editPatchList(patchFile, (rows) => updateMcpConfig(rows, id, /** @type {McpServerConfig} */ (config)));
      });
      return { id };
    },
    /** @param {GroupIdArgs | undefined} [args] */
    ...
    /** @param {McpProbeArgs | undefined} [args] */
    async mcpProbe(args) {
      ...
      if (args?.config !== null && typeof args?.config === 'object') {
        ...
        config = /** @type {McpServerConfig} */ (
          pickServerConfig(/** @type {Record<string, unknown>} */ (args.config)));
      } else {
  ...
  // lib/tool-schemas.js (pure, closure-free). Runtime dependencies (js-yaml,
  // the MCP SDK) are lazy-loaded by lib/patch.js and lib/probe.js, so
  // registering tools never requires them.

  /** @type {(() => void)[]} */
  const toolDisposers = [];
  /**
   * @param {ToolDefinitionInput} definition
   * @returns {void}
   */
  ...
  tool({
    ...
    output: {
      ...
      render: (args, value) => [{ type: 'text', text: `Group "${args.name}" created (id ${(/** @type {{ id: string }} */ (value)).id}).` }],
    },
    async execute(args) {
      return api.groupsCreate(/** @type {GroupCreateArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.groupsDelete(/** @type {GroupIdArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.groupsRename(/** @type {GroupRenameArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.groupsSetEnabled(/** @type {GroupSetEnabledArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    output: {
      ...
      render: (args, value) => [{ type: 'text', text: `Group ${args.id} now has skills: ${(/** @type {{ skills: string[] }} */ (value)).skills.join(', ')}.` }],
    },
    async execute(args) {
      return api.groupsAddSkill(/** @type {GroupSkillsArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.groupsRemoveSkill(/** @type {GroupSkillsArgs} */ (args));
    },
  });
   ...
   * @param {ToolExecContext | undefined} exec
   * @returns {string}
   */
  ...
  tool({
    ...
    output: {
      schema: sessionStateOutput,
      render: (args, value) => {
        const state = /** @type {import('./types.js').SessionStateResult} */ (value);
        return [{
          type: 'text',
    ...
    async execute(args, exec) {
      return api.sessionSet({
        ...
        enabledGroupIds: /** @type {string[] | null} */ (args.enabledGroupIds),
      });
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.mcpToggle(/** @type {McpToggleArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.mcpAdd(/** @type {McpAddArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.mcpUpdate(/** @type {McpAddArgs} */ (args));
    },
  });
  ...
  tool({
    ...
    async execute(args) {
      return api.mcpRemove(/** @type {GroupIdArgs} */ (args));
    },
  });
  ...
  const rpcMethods = {
    ...
    'manager.skills.list': async (args) => api.skillsList(/** @type {SkillsListArgs} */ (args)),
    'manager.groups.create': async (args) => api.groupsCreate(/** @type {GroupCreateArgs} */ (args)),
    'manager.groups.rename': async (args) => api.groupsRename(/** @type {GroupRenameArgs} */ (args)),
    'manager.groups.delete': async (args) => api.groupsDelete(/** @type {GroupIdArgs} */ (args)),
    'manager.groups.setEnabled': async (args) => api.groupsSetEnabled(/** @type {GroupSetEnabledArgs} */ (args)),
    'manager.groups.addSkill': async (args) => api.groupsAddSkill(/** @type {GroupSkillsArgs} */ (args)),
    'manager.groups.removeSkill': async (args) => api.groupsRemoveSkill(/** @type {GroupSkillsArgs} */ (args)),
    'manager.session.get': async (args) => api.sessionGet(/** @type {SessionGetArgs} */ (args)),
    'manager.session.set': async (args) => api.sessionSet(/** @type {SessionSetArgs} */ (args)),
    'manager.mcp.list': async () => api.mcpList(),
    'manager.mcp.toggle': async (args) => api.mcpToggle(/** @type {McpToggleArgs} */ (args)),
    'manager.mcp.add': async (args) => api.mcpAdd(/** @type {McpAddArgs} */ (args)),
    'manager.mcp.update': async (args) => api.mcpUpdate(/** @type {McpAddArgs} */ (args)),
    'manager.mcp.remove': async (args) => api.mcpRemove(/** @type {GroupIdArgs} */ (args)),
    'manager.mcp.probe': async (args) => api.mcpProbe(/** @type {McpProbeArgs} */ (args)),
  };
  ...
  const registerWebSurface = () => {
    ...
    const webServer = /** @type {WebServerService | undefined} */ (
      ctx.get(WEB_SERVER_KEYS[0]) ?? ctx.get(WEB_SERVER_KEYS[1]));
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const value = await handler(/** @type {Record<string, unknown>} */ (body.args ?? {}));
          ...
          const err = /** @type {Error & { code?: unknown, fields?: Record<string, string> | undefined }} */ (
  ...
  ctx.on('internal/service', (serviceName) => {
    if (WEB_SERVER_KEYS.includes(/** @type {'webServer' | 'httpServer'} */ (serviceName))) registerWebSurface();
  });
